...
class QTranBase(nn.Module):
    def __init__(self, args):
        ...
        self.arch = self.args.config["qtran_arch"]  # QTran architecture
        self.hidden_dim = self.args.config["hidden_dim"]
        self.embed_dim = args.config["mixing_embed_dim"]
        ...
        if self.arch == "coma_critic":
            ...
            q_input_size = self.state_dim + self.hidden_dim + self.n_actions
        else:
            raise Exception("{} is not a valid QTran architecture".format(self.arch))

        if self.args.config["network_size"] == "small":
