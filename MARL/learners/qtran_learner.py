...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.mixer = None
        if args.config["mixer"] == "qtran_base":
            self.mixer = QTranBase(args)
        elif args.config["mixer"] == "qtran_alt":
        ...
        self.optimiser = RMSprop(
            params=self.params,
            lr=args.config["lr"],
            alpha=args.config["optim_alpha"],
            eps=args.config["optim_eps"],
        )
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.config["mixer"] == "qtran_base":
            # -- TD Loss --
            ...
            if self.args.config["use_double_q"]:
        ...
        elif self.args.config["mixer"] == "qtran_alt":
            raise Exception("Not supported yet.")

        loss = (
            td_loss
            + self.args.config["opt_loss"] * opt_loss
            + self.args.config["nopt_loss"] * nopt_loss
        )
        ...
        self.optimiser.step()
        if (episode_num - self.last_target_update_episode) / self.args.config[
    ...
    def load_models(self, path, postfix=""):
        ...
        if self.mixer is not None:
            self.mixer.load_state_dict(
                th.load(
                    "{}/mixer".format(path) + postfix + ".th",
                    map_location=lambda storage, loc: storage,
                )
            )
        self.optimiser.load_state_dict(
            th.load(
                "{}/opt".format(path) + postfix + ".th",
                map_location=lambda storage, loc: storage,
            )
        )
