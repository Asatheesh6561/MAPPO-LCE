...
from MARL.modules.critics.maddpg import MADDPGCritic
import torch as th
...
class MADDPGLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.agent_optimiser = Adam(params=self.agent_params, lr=self.args.config['lr'])
        self.critic_optimiser = Adam(params=self.critic_params, lr=self.args.config['lr'])
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        actions = actions.view(batch_size, -1, 1, self.n_agents).expand(-1, -1, self.n_agents, -1)
        ...
        for t in range(1, batch.max_seq_length):
            agent_target_outs = self.target_mac.target_actions(batch, t).sum(dim=-1)
            target_actions.append(agent_target_outs)
        target_actions = th.stack(target_actions, dim=1)  # Concat over time
        target_actions = target_actions.view(batch_size, -1, 1, self.n_agents).expand(-1, -1, self.n_agents, -1)
        ...
        targets = rewards.reshape(-1, 1) + self.args.config['gamma'] * (1 - terminated.reshape(-1, 1)) * target_vals.reshape(-1, 1).detach()
        ...
        loss.backward()
        critic_grad_norm = th.nn.utils.clip_grad_norm_(self.critic_params, self.args.config['grad_norm_clip'])
        ...
        for t in range(batch.max_seq_length-1):
            ...
            actions.append(gumbel_softmax(pi, hard=True).sum(dim=-1))
            pis.append(pi)
        ...
        for i in range(self.n_agents):
            temp_action = th.split(actions[:, :, i, :], 1, dim=2)
            actions_i = []
        ...
        pg_loss = (q * mask).mean() + self.args.config['reg'] * (pis ** 2).mean()

        # Optimise agents
        self.agent_optimiser.zero_grad()
        pg_loss.backward()
        agent_grad_norm = th.nn.utils.clip_grad_norm_(self.agent_params, self.args.config['grad_norm_clip'])
        self.agent_optimiser.step()

        if self.args.config['target_update_interval_or_tau'] > 1 and (episode_num - self.last_target_update_episode) / self.args.config['target_update_interval_or_tau'] >= 1.0:
            self._update_targets_hard()
            self.last_target_update_episode = episode_num
        elif self.args.config['target_update_interval_or_tau'] <= 1.0:
            self._update_targets_soft(self.args.config['target_update_interval_or_tau'])
    ...
    def save_models(self, path, postfix=""):
        ...
        th.save(self.critic.state_dict(), "{}/critic".format(path) + postfix + ".th")
        th.save(self.agent_optimiser.state_dict(), "{}/agent_opt".format(path) + postfix + ".th")
        th.save(self.critic_optimiser.state_dict(), "{}/critic_opt".format(path) + postfix + ".th")
