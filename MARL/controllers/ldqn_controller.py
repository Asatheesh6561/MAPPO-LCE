...
class LDQNMAC:
    ...
    def select_actions(
        self, ep_batch, t_ep, t_env, lbdas=None, bs=slice(None), test_mode=False
    ):
        # Only select actions for the selected batch elements in bs
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        agent_outputs = self.forward(
            ep_batch, t_ep, t_env, lbdas=lbdas, test_mode=test_mode
        )
    ...
    def forward(self, ep_batch, t_ep, t_env, lbdas=None, test_mode=False):
        ...
        agent_outs, self.hidden_states = self.agent(
            agent_inputs, self.hidden_states, lbdas
        )
